...
while True:
    ...
    # draw a center line in the frame
    centerX = frame.shape[1] // 2
    cv2.line(frame, (centerX, 0), (centerX, frame.shape[0]), (255, 0, 0), 2)
    ...
    if len(cnts) > 0:
        ...
        if radius > 10:
            ...
            # check if the ball is in the left or right side of the frame
            if center[0] < centerX:
                goal = True
            else:
                goal = False
    ...
    # show the goal status
    status_text = "Goal!" if goal else "No Goal"
    cv2.putText(frame, status_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 0) if goal else (0, 0, 255), 3)
